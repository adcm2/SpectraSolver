...
class MatrixReplaceFT : public Eigen::EigenBase<MatrixReplaceFT<MRScalar>> {
   public:
    ...
    MatrixReplaceFT(const MATRIX &a0, const MATRIX &a1, const MATRIX &a2,
                    const MRScalar w)
        : _mat_a0{a0}, _mat_a1{a1}, _mat_a2{a2}, _w{w} {};

    void UpdateFrequency(MRScalar winp) { _w = winp; };
    ...
    const MRScalar coeff(int idxi, int idxj) const {
        ...
        tmp += _mat_a0.coeff(idxi, idxj) + _mat_a1.coeff(idxi, idxj) * _w -
               _mat_a2.coeff(idxi, idxj) * _w * _w;
        return tmp;
    }
...
struct generic_product_impl<MatrixReplaceFT<MRScalar>, Rhs, SparseShape,
    ...
    static void scaleAndAddTo(Dest &dst, const MatrixReplaceFT<MRScalar> &lhs,
        ...
        dst.noalias() += alpha * (lhs.a0() * rhs + lhs.w() * (lhs.a1() * rhs) -
                                  lhs.w() * lhs.w() * (lhs.a2() * rhs));
        // dst.noalias() += alpha *
        //                  ((lhs.a0()) + lhs.w() * ((lhs.a1())) +
        //                   lhs.w() * lhs.w() * ((lhs.a2()))) *
        //                  rhs;
