...
class basic_couple {
   public:
    ...
    basic_couple(matcd &, matcd &, matcd &);
   ...
   private:
    matcd m_a0, m_a1, m_a2;
    std::size_t m_nelem;
};

basic_couple::basic_couple(matcd &a0, matcd &a1, matcd &a2) {
    m_a0 = a0;
    m_a1 = a1;
    m_a2 = a2;
    m_nelem = a0.rows();
...
class couplematrix {
   public:
    ...
    // constructor if reading in normal mode coupling matrices, receiver and
    // source vectors etc from binary files that go into the old code
    couplematrix(std::string, std::string, std::string);

    // not sure if i need m_ww, m_ww
