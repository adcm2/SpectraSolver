...
{
private:
    std::chrono::time_point<std::chrono::_V2::system_clock, std::chrono::_V2::system_clock::duration> _start, _stop;
    std::chrono::__enable_if_is_duration<std::chrono::microseconds> _duration;

public:
    Timer() : _start{high_resolution_clock::now()}, _stop{high_resolution_clock::now()}, _duration{duration_cast<microseconds>(_stop - _start)} {};
    ...
    void stop(const std::string &strmessage)
    {
        ...
        std::cout << strmessage << ": " << _duration.count() / 1000000.0 << " seconds\n";
    };
};
